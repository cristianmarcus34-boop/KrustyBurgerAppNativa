...
async function manejarWebhook(body: WebhookPayload): Promise<RespuestaAPI> {
    ...
    if (topic === 'payment') {
        // Buscar la transacción por mp_payment_id
...
serve(async (req: Request) => {
    try {
        ...
        const path = url.pathname.split('/').pop() // Obtener la última parte de la ruta
        ...
        // ==========================================
        // CREAR PREFERENCIA - App → Supabase → MP
        ...
        // ==========================================
        // VERIFICAR ESTADO - App → Supabase
        // GET /estado?pedidoId=123
        // ==========================================
        if (path === 'estado') {
            if (req.method !== 'GET') {
                return new Response('Method not allowed', { status: 405 })
